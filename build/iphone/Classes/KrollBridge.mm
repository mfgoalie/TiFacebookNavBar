...
NSString * FacebookNavBarViewExample$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation FacebookNavBarViewExampleObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_facebooknavbarviewexample);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create FacebookNavBarViewExample global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_facebooknavbarviewexample = [[FacebookNavBarViewExampleObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_facebooknavbarviewexample];
	
	NSString *_facebooknavbarviewexampleNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _facebooknavbarviewexampleNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_facebooknavbarviewexample valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:FacebookNavBarViewExample$ModuleRequireFormat,code];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	if (data!=nil)
	{
		...
		if (![module respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			@throw [NSException exceptionWithName:@"org.facebooknavbarviewexample.kroll" reason:[NSString stringWithFormat:@"Module \"%@\" failed to leave a valid exports object",path] userInfo:nil];
		}
	...
	@throw [NSException exceptionWithName:@"org.facebooknavbarviewexample.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
